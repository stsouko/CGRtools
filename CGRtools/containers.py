...
class MoleculeContainer(Graph):
    ...
    def pickle(self, stereo=False, compress=True):
        ...
        """
        g = Graph()
        g.graph['meta'] = self.meta
        if compress and not self.get_center_atoms(stereo=stereo):
            g.graph['s_only'] = True
            node_marks = self.__node_s
            edge_marks = self.__edge_s
        else:
            node_marks = self.__node_sp
            edge_marks = self.__edge_sp
        ...
        return node_link_data(g, attrs=self.__attrs)
    ...
    def unpickle(cls, data):
        ...
        if g.graph.pop('s_only', None):
            ...
            for *_, a in g.edges(data=True):
                a.update(cls.__attr_sp_clone(a, cls.__edge_marks))
        else:
            g.fix_sp_marks()
        return g
    ...
    def get_fear(self, weights=None, isotope=False, stereo=False, hyb=False, element=True):
        ...
        """
        return get_cgr_string(self, weights or get_morgan(self, isotope=isotope, stereo=stereo, element=element),
    ...
    def get_center_atoms(self, stereo=False):
        ...
        for n, node_attr in self.nodes(data=True):
            if any(node_attr.get(k) != node_attr.get(l) for k, l in (self.__rcn_stereo if stereo else self.__rcn)):
                nodes.add(n)

        for *n, node_attr in self.edges(data=True):
            if any(node_attr.get(k) != node_attr.get(l) for k, l in (self.__rce_stereo if stereo else self.__rce)):
                nodes.update(n)
    ...
    def get_morgan(self, isotope=False, stereo=False, element=True):
        return get_morgan(self, isotope=isotope, stereo=stereo, element=element)
    ...
    def fix_sp_marks(self, copy=False, nodes_bunch=None, edges_bunch=None):
        ...
        for *_, a in g.edges(nbunch=edges_bunch, data=True):
            a.update(self.__attr_renew(a, self.__edge_marks))
    ...
    __node_marks = [('s_%s' % mark, 'p_%s' % mark, 'sp_%s' % mark) for mark in ('neighbors', 'hyb', 'stereo', 'charge')]
    __edge_marks = [('s_%s' % mark, 'p_%s' % mark, 'sp_%s' % mark) for mark in ('bond', 'stereo')]

    __tmp_node = ['element', 'isotope', 'mark', 'x', 'y', 'z']
    __node_s = [x for x, *_ in __node_marks] + __tmp_node
    __edge_s = [x for x, *_ in __edge_marks]
    __node_sp = [y for x in __node_marks for y in x[:2]] + __tmp_node
    __edge_sp = [y for x in __edge_marks for y in x[:2]]

    __tmp_rcs = [('s_stereo', 'p_stereo')]
    __rcn = [('s_charge', 'p_charge')]
    __rce = [('s_bond', 'p_bond')]
    __rcn_stereo = __tmp_rcs + __rcn
    __rce_stereo = __tmp_rcs + __rce
